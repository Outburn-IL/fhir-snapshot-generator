...
const omitKeys = async (obj: object, okeys: string[]): Promise<object> => {
  const res = await jsonata('$in.$sift($, function($v, $k) {($k in $okeys)=false})').evaluate({}, { in: obj, okeys });
  return res;
};

const startsWith = (str: string | undefined, startStr: string): boolean | undefined => {
  // undefined inputs always return undefined
  if (typeof str === 'undefined') return undefined;
  return str.startsWith(startStr);
};

const endsWith = (str: string | undefined, endStr: string): boolean | undefined => {
  // undefined inputs always return undefined
  if (typeof str === 'undefined') return undefined;
...
const expressions = {
  ...
  mergeElementDefinition: jsonata(`
    $count($arrayOfElements) = 1 ? $arrayOfElements[0] :
    (
      /* this function takes an array of elements and merges them into a single element definition */
      $count($arrayOfElements.base.path[$endsWith($,'.assigner')]) > 0 ? $info($arrayOfElements[].type[]);
      ...
      $res := $merge($append($arrayOfElements, [$conditionsObj, $constraintsObj]));
      $endsWith($res.base.path, '.assigner') ? $info(($res.type[].targetProfile[]));
      $res;
    )
  `),
...
const getStructureDefinition = async (logger: ILogger, getRawDefinition: getStructureDefinitionFunction, definitionId: string, pkg?: string | PackageIdentifier) => {
  const rawDef = await getRawDefinition(definitionId, pkg);
...
const mergeElementDefinition = async (arrayOfElements: any | any[]) => {
  return await expressions.mergeElementDefinition.evaluate({}, { arrayOfElements, info: console.info, endsWith });
};
...
export const generateSnapshot = async (logger: ILogger, getRawDefinition: getStructureDefinitionFunction, getSnapshot: getStructureDefinitionFunction, profileId: string, pkg?: PackageIdentifier | string ) => {
  return await expressions.generateSnapshot.evaluate({},
    {
      ...
      getStructureDefinition: async (profileId: string) => await getStructureDefinition(logger, getRawDefinition, profileId, pkg),
      changeRoot,
      removeSliceFromId,
      getLevel,
      fixDiffIds,
      ...
      getSnapshot: async (profileId: string) => await getSnapshot(profileId, pkg),
